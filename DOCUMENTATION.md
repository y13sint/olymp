...
| № | Раздел | Требование ТЗ |
|---|--------|---------------|
| — | [Титульный лист](#титульный-лист) | ✅ п.1 |
| 1 | [Обоснование выбора языка и программных средств](#1-обоснование-выбора-языка-программирования-и-программных-средств) | ✅ п.2 |
| 2 | [Структурная и функциональная схемы](#2-структурная-и-функциональная-схемы-программного-продукта) | ✅ п.3 |
| 3 | [Блок-схема работы основного алгоритма](#3-блок-схема-работы-основного-алгоритма) | ✅ п.4 |
| 4 | [Описание и аргументация выбора СУБД](#4-описание-особенностей-и-аргументация-выбранного-типа-субд) | ✅ п.5 |
| 5 | [Схема базы данных](#5-схема-базы-данных) | ✅ п.6 |
| 6 | [Статистика проекта](#6-статистика-проекта) | — |
| 7 | [Тестовые пользователи](#7-тестовые-пользователи) | — |
| 8 | [Ссылки (репозиторий, видео)](#8-ссылки) | ✅ п.7 |
...
```
             ...
             │ Unauthorized    │              │ + SELECT FOR UPDATE │
             └────────┬────────┘              │   (блокировка User) │
                      │                       └──────────┬──────────┘
                      │                                  │
                      │                       ┌──────────▼──────────┐
                      ...
                      │                   │                            │
                      │          ┌────────▼────────┐         ┌─────────▼─────────┐
                      │          │ Ошибка 400:     │         │ Проверка активной │
                      │          │ "Уже заказано"  │         │    подписки       │
                      │          └────────┬────────┘         └─────────┬─────────┘
                      │                   │                            │
                      │                   │              ┌─────────────▼─────────────┐
                      │                   │              │   Есть подписка на       │
                      ...
                      │                   │              │                           │
                      │                   │    ┌─────────▼─────────┐    ┌────────────▼────────────┐
                      │                   │    │ Создать MealPickup│    │  Проверка баланса       │
                      │                   │    │ (оплата подпиской)│    │  >= цена блюда          │
                      │                   │    └─────────┬─────────┘    └────────────┬────────────┘
                      │                   │              │               ┌───────────▼───────────┐
                      │                   │              │               │  Баланс достаточен?   │
                      │                   │              │               └───────────┬───────────┘
                      ...
                      │                   │              │    │ "Недостаточно     │  │ Создать MealPickup    │
                      │                   │              │    │  средств"         │  │ (оплата с баланса)    │
                      │                   │              │    └─────────┬─────────┘  └───────────┬───────────┘
                      │                   │              │              │                        │
                      │                   │              │              │                        │
                      │                   │              └──────────────┼────────────────────────┘
...
```
                    ...
                    └──────────────────┬──────────────────┘
                      ...
                      │                                  │
           ┌──────────▼──────────┐           ┌──────────▼──────────┐
           │ inventoryService.   │           │ request.update({    │
           │ restockFromRequest  │           │   status: 'rejected',│
           │ (product, quantity, │           │   approvedBy: adminId│
           │  transaction)       │           │ })                   │
           └──────────┬──────────┘           └──────────┬──────────┘
                      │                                  │
           ┌──────────▼──────────┐                       │
...
| Транзакции | В финансовых операциях |
